...
runTest(int argc, char** argv)
{
    ...
    // Compare results
    {
        ...
        for (int i = 0; i < VECTOR_ELEMENTS; i++) {
            if (reference[i] != C[i]) {
				printf("Unexpected result in C[%d] = %d\n", i, C[i]);
                correct = false;
                break;
            }
        }
	...
	printf("Se ha conseguido un factor de aceleracion %gx utilizando CUDA (considerando transferencias) \n", (cpu_end_time - cpu_start_time) / (gpu_end_time_trans - gpu_start_time_trans) );
	unsigned long long a = grid.x * grid.y * grid.z * block.x * block.y * block.z;
	printf("Llamadas: %llu\n", a);
    // clean up memory
    free(A);
    free(B); 
